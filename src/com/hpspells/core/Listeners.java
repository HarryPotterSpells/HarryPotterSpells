...
public class Listeners implements Listener {
    ...
    public void PIE(PlayerInteractEvent e) {
        if (e.getPlayer().hasPermission(CAST_SPELLS) && HPS.Wand.isWand(e.getPlayer().getItemInHand())) {
            ...
            Wizard wizard = WizardManager.getWizard(e.getPlayer().getUniqueId());
            ...
            if (e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) {
                Integer knows = wizard.getKnownSpellsList().size() - 1, cur = HPS.SpellManager.getCurrentSpellPosition(e.getPlayer().getUniqueId()), neww;

                if (wizard.getKnownSpellsList().isEmpty() || cur == null) {
                ...
                try {
                    HPS.PM.newSpell(e.getPlayer(), HPS.SpellManager.setCurrentSpellPosition(e.getPlayer().getUniqueId(), neww).getName());
            ...
            if (e.getAction() == Action.LEFT_CLICK_AIR || e.getAction() == Action.LEFT_CLICK_BLOCK) {
                Spell currentSpell = HPS.SpellManager.getCurrentSpell(e.getPlayer().getUniqueId());
                if (currentSpell != null) {
                    HPS.SpellManager.cleverCast(e.getPlayer(), currentSpell);
                } else {
                	HPS.PM.log(Level.INFO, "CurrentSpell is null");
                }
            }
    ...
    public void onPreLogin(AsyncPlayerPreLoginEvent event) {
        WizardManager.loadWizard(event.getUniqueId(), event.getName());
    }
    
    @EventHandler
    public void onQuit(PlayerQuitEvent event) {
        WizardManager.saveWizard(event.getPlayer());
    }
}
